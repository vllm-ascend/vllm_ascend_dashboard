...
## 概述
将看板聚合数据（Nightly 流水线 / PR 流水线 / 模型验证 / 资源看板 / 测试看板 / GitHub 动态）转化为一份**正面、系统、可扫读**的社区每日运行报告，让社区 Maintainer 和贡献者用 30 秒了解项目整体进展、昨日成果和需关注事项。
...
| **系统性** | 六大维度（Nightly/PR/模型/资源/测试/GitHub）均需覆盖，有数据的报数据，无数据的简要说明 |
...
| 状态 | 判定标准 |
|------|----------|
| 🟢 **正常** | Nightly 成功率≥90% / 测试通过率≥95% / 无关键模型失败 / 资源利用率<90% |
| 🟡 **轻微波动** | Nightly 成功率 70–90% / 个别测试失败 / 资源利用率 90–95% / 积压轻度上升 |
| 🔴 **高危异常** | Nightly 成功率<70% / 连续失败≥2 / 关键模型失败 / 资源利用率>95% / 积压严重 |
...
> {{one_sentence_overview}}  ← 一句话概览：昨日新增XX PR，合入XX，Issue XX个，Nightly成功率XX%，资源利用率XX%，测试通过率XX%，整体健康度【状态】

## ⚡ 当日效能极速概览

当日工程健康度：【正常/轻微波动/高危异常】，相较昨日【回升/持平/恶化】
- Nightly 流水线：{{ci_status}}（成功率 {{success_rate}}%）
...
### 第三步：撰写报告
- **一句话概览**：昨日新增X PR，合入X，Issue X个，Nightly成功率X%，资源利用率X%，测试通过率X%，健康度【状态】
