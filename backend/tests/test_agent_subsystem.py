...
def test_explicit_system_prompt_has_priority_over_skill():
    ...
    assert service._build_system_prompt("scope", "explicit prompt", []) == "explicit prompt"
    assert service._build_system_prompt("scope", "", []) == "skill prompt"
...
def test_failure_analysis_tools_include_active_memory_search():
    assert agent_tools.search_memory in agent_tools.FAILURE_ANALYSIS_TOOLS
