...
1. **Annotations（最优先证据）** — [列出关键 annotation，逐条分析含义和影响]
2. **历史运行对比** — [对比同 workflow 最近几次运行中此 job 的状态（success/skipped/failure），说明是否首次执行或偶发失败]
3. **失败步骤特征** — [分析失败步骤的名称、类型、耗时，推断问题性质]
4. **Commit 对比** — [列出上次成功运行与本次失败之间的 head commit 变化，说明是否涉及代码变更]
...
1. 报告必须严格按照上方「CI 失败分析报告模板」的章节结构输出，包含：基本信息表、问题分类、根因分析（证据来源+根因链路）、影响范围、修复建议表、关联PR、结论

2. 证据来源必须按优先级排序：Annotations > 历史运行对比 > 失败步骤特征 > Commit 对比

3. 根因链路必须用代码块格式的因果链条可视化呈现
