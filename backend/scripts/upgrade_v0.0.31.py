"""Database upgrade script v0.0.31
...
async def upgrade():
    print("\n" + "=" * 60)
    print("  Starting upgrade to v0.0.31")
    print("=" * 60 + "\n")
    ...
    async with SessionLocal() as db:
        try:
            ...
            result = await db.execute(text(
                "SELECT COUNT(*) FROM database_versions WHERE version = '0.0.31'"
            ))
            count = result.scalar()
            if count == 0:
                await db.execute(text(
                    """INSERT INTO database_versions (version, description, applied_at)
                       VALUES ('0.0.31', :description, :applied_at)"""
                ), {"description": DESCRIPTION, "applied_at": datetime.now()})
                await db.commit()
                print("  [OK] Version v0.0.31 recorded")
            else:
                print("  [SKIP] Version v0.0.31 already recorded")

            print("\n" + "=" * 60)
            print("  Upgrade to v0.0.31 completed!")
            print("=" * 60 + "\n")
